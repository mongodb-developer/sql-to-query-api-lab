...
👐 Maintain a `review_stats` collection that contains the total number of reviews per book.
👐 Update the `borrowed_books_summary` collection by storing the total borrowed count for each book.

---

🚀 **Next Up: Optimizing Aggregation Pipelines!**
