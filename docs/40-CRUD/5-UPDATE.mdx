...
<details>
  <summary>Answer</summary>
    ...
    <TabItem value="go" label="Go">
      <div>
        ```go
        filter := bson.D{{Key: "title", Value: "Treasure of the Sun"}}
        update := bson.D{{Key: "$set", Value: bson.D{{Key: "pages", Value: int32(449)}}}}

        result, err := booksCollection.UpdateOne(context.TODO(), filter, update)
        if err != nil {
            log.Fatal(err)
        }
