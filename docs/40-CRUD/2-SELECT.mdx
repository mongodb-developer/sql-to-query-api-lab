...
<details>
  ...
  <Tabs groupId="challenges" defaultValue={"javascript"}>
    ...
    <TabItem value="go" label="Go">
      <div>
        ```go
        projection := bson.D{{Key: "_id", Value: 0}, {Key: "title", Value: 1}}
        opts := options.Find().SetProjection(projection).SetLimit(10)

        cursor, err := booksCollection.Find(context.TODO(), bson.D{}, opts)
        if err != nil {
            log.Fatal(err)
        }
        var results []Book
...
<details>
  ...
  <Tabs groupId="challenges" defaultValue={"javascript"}>
    ...
    <TabItem value="go" label="Go">
      <div>
        ```go
        filter := bson.D{{Key: "genres", Value: "History"}}
        projection := bson.D{{Key: "_id", Value: 0}, {Key: "authors", Value: 0}}
        opts := options.Find().SetProjection(projection).SetLimit(10)

        cursor, err := booksCollection.Find(context.TODO(), filter, opts)
        if err != nil {
            log.Fatal(err)
        }
        var results []bson.M
